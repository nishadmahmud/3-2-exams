...
export function getSemesters() {
  ...
  if (!config || !config.semesters) return [];
  return Object.keys(config.semesters);
}

export function getSemesterCourses(semester) {
  const config = getCourseConfig();
  if (!config || !config.semesters || !config.semesters[semester]) return {};
  return config.semesters[semester].courses || {};
}

export function getCourseInfo(semester, courseId) {
  const courses = getSemesterCourses(semester);
  return courses[courseId] || null;
}

export function getLectures(semester, courseId) {
  ...
  return files.map(file => {
    ...
    let subtitle = courseInfo.lectureSubtitles?.[title] || title;
    if (!isPdf) {
...
export function getLectureBySlug(semester, courseId, slugArray) {
...
export function getQuestions(semester, courseId) {
  const courseInfo = getCourseInfo(semester, courseId);
  if (!courseInfo) return [];
...
export function getQuestionBySlug(semester, courseId, slugArray) {
  const decodedSlug = slugArray.map(s => decodeURIComponent(s));
  const courseInfo = getCourseInfo(semester, courseId);
  if (!courseInfo) return null;
...
export function getExtraBySlug(semester, courseId, slugArray) {
  const decodedSlug = slugArray.map(s => decodeURIComponent(s));
  const courseInfo = getCourseInfo(semester, courseId);
  if (!courseInfo) return null;
